...
TEST_F(msturn, read_many)
{
  ...
  auto failed_count = msg.read_many(error, failed,
    turner::msturn::username.value_ref(username),
    turner::msturn::software.value_ref(software)
  );
...
TEST_F(msturn, read_many_unknown_comprehension_required)
{
  auto data = wire_data(MSTURN(),
    "\x00\x06"          // username
    "\x00\x08"          // Length
  ...
  EXPECT_EQ(turner::msturn::username.type, failed[0]);
}


TEST_F(msturn, read_many_unknown_comprehension_required_overflow)
{
  auto data = wire_data(MSTURN(),
    "\x00\x06"          // username
    "\x00\x08"          // Length
...
TEST_F(msturn, read_many_missing)
{
  auto data = wire_data(MSTURN(),
    "\x80\x22"          // software
    "\x00\x08"          // Length
...
TEST_F(msturn, read_many_missing_overflow)
{
  auto data = wire_data(MSTURN(),
    "\x00\x06"          // username
    "\x00\x08"          // Length
  ...
  std::string_view username, software;
  turner::msturn::alternate_server_t::value_t alternate_server;
  ...
  auto failed_count = msg.read_many(error, failed,
    turner::msturn::username.value_ref(username),
    turner::msturn::software.value_ref(software),
    turner::msturn::alternate_server.value_ref(alternate_server)
  );
  EXPECT_EQ(turner::errc::attribute_not_found, error);
  ASSERT_EQ(1U, failed_count);
  EXPECT_EQ(turner::msturn::software.type, failed[0]);
}


TEST_F(msturn, read_many_unexpected_attribute_value)
{
  auto data = wire_data(MSTURN(),
    "\x80\x20"          // xor_mapped_address
    "\x00\x08"          // Length
  ...
  turner::msturn::xor_mapped_address_t::value_t xor_mapped_address;

  std::error_code error;
  uint16_t failed[4];
  auto failed_count = msg.read_many(error, failed,
    turner::msturn::xor_mapped_address.value_ref(xor_mapped_address)
  );
  EXPECT_EQ(turner::errc::unexpected_attribute_value, error);
  EXPECT_EQ(0U, failed_count);
}
...
TEST_F(msturn, read_many_insufficient_payload_data)
{
  auto data = wire_data(MSTURN(),
    "\x80\x20"          // XOR-MAPPED-ADDRESS
    "\x00\x08"          // Length
    "\x00\xff"          // Value (invalid)
